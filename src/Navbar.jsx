...
// map your exact logo strings to icon components
const iconMap = {
    GrCatalog: GrCatalog,
...
const Navbar = ({ hameburger, setHameburger }) => {
    ...
    return (
        <>
            ...
            <div className="flex border-2 border-gray-200 w-full justify-between items-center pb-1.5 h-18 pr-6 relative">
                <div className="flex items-center">
                    <img
                        ...
                        width={250}
                        height={150}
                        className="h-30 hidden md:block"
                        alt="logo"
                    />
            ...
            <div
                className={`
        absolute top-18 left-0 min-h-screen w-60 bg-[#242D37] text-white z-50
            ...
            >
                ...
                <div className="">
                    {NavaData.map((item, index) => {
                        const Icon = iconMap[item.logo]; // use your logo string

                        return (
                            <div key={index} className="">
                                <div
                                    className={`
                                   flex items-center w-full h-10 pl-5 pr-3 cursor-pointer
      ...
      ${openItems[index] ? "text-blue-400" : "text-white"}
    `}
                                    >
      ...
      ${openItems[index] ? "rotate-180 text-blue-400" : "text-white"}
    `}
                                    />
                                </div>
                                ...
                                {openItems[index] && (
                                    <ul className="text-xs ml-3 space-y-1 mt-1">
                                        {item.subHeading?.map((sub, i) => (
                                            <div className="flex gap-3 text-[15px] text-[#696969] cursor-pointer hover:text-gray-300 transition-all duration-300 hover:translate-x-3">
                                                <MdKeyboardDoubleArrowRight />
                                                <li
                                                    key={i}
                                                    className="cursor-pointer hover:text-gray-300 transition-all duration-300 hover:translate-x-3"
                                                >
                                                    {sub}
                                                </li>
